...
function install() {
    ...
    while IFS= read -r file; do
        ...
        if grep -q "^$pkg$" "$exclude_list_file" && ! grep -q "^$pkg$" "$include_list_file"; then
            ...
            # 解压data.tar文件到输出目录
            mkdir "$data_list_dir"
            tar -xvf "$data_file" -C "$data_list_dir" | sed 's#./usr#./#' | sed "s#^./#$path/#" >>"$deb_trace_dir/$(basename "$file").list"
            rm "$data_file"
            ...
            find "$data_list_dir" -type l | while IFS= read -r file; do
                ...
                # 如果指向的路径以/lib开头, 并且文件不存在, 则添加 $path 前缀
                # 部分 dev 包会创建 so 文件的绝对链接指向 /lib 目录下
                # + 处理 /etc
                if echo "$linkTarget" | grep -q '^/lib\|^/etc' && ! [ -f "$linkTarget" ]; then
                    ln -sf "$path$linkTarget" "$file"
                    echo "    FIX LINK" "$linkTarget" "=>" "$path$linkTarget"
                fi
            done
            # 修复动态库的RUNPATH
            ...
            # 复制/lib,/bin,/usr目录
            # + 复制/etc目录
            cp -rP "$data_list_dir/lib" "$path" 2>/dev/null || true
            cp -rP "$data_list_dir/bin" "$path" 2>/dev/null || true
            cp -rP "$data_list_dir/etc" "$path" 2>/dev/null || true
            cp -rP "$data_list_dir"/usr/* "$path" || true
            rm -r "$data_list_dir"
        fi
    done <"$deb_list_file"
